...
#    print("Loading training batch")
    ...
    # Indexes for randomly selected Abnormal Videos
    Abnor_list_iter = np.random.permutation(Num_abnormal)
    Abnor_list_iter = Abnor_list_iter[int(Num_abnormal-n_exp):] #30 videos
     
    # Indexes for randomly selected Normal Videos
    Norm_list_iter = np.random.permutation(Num_Normal)
    Norm_list_iter = Norm_list_iter[int(Num_Normal-n_exp):]     #30 videos
    ...
    for iv in Abnor_list_iter:
        ...
        #VideoPath = os.path.join(AllVideos_Path, All_Videos[iv])  DEBUG
        VideoPath = AllVideos_Path #DEBUG
          
        '''
        list_c3d_txt = listdir(VideoPath)  # list_c3d_txt[0] --> 'Abuse001_x264.txt', list_c3d_txt[iv]
        '''
        list_c3d_txt = listdir(VideoPath) 
        #Read each txt file
        f = open(os.path.join(VideoPath, list_c3d_txt[iv]), 'r') # DEBUG
        #f = open(VideoPath, "r")  #DEBUG
        words = f.read().split()
        ...
        # Please see Save_C3DFeatures_32Segments.m as well
        ...
        if Video_count > 0:
            ...
            #Vertically stack every video's (total 30 per abnormal) feature per batch (32, 4096) --> (960, 4096)
            
        print("Abnormal Features loaded, Video: ", list_c3d_txt[iv])
    ...
    for iv in Norm_list_iter:
        ...
        print("Normal Features loaded, Video: ", list_c3d_txt[iv])
    ...
    for iv in range(0, 32*batchsize):
            if iv< th_loop1:
                AllLabels[iv] = int(0)  # All segments of abnormal videos are labeled 0.  This will be used in custom_objective to keep track of normal and abnormal videos indexes.
            if iv > th_loop2:
                AllLabels[iv] = int(1)   # All segments of Normal videos are labeled 1. This will be used in custom_objective to keep track of normal and abnormal videos indexes.
...
def custom_objective(y_true, y_pred):
    ...
    sub_l2 = sub_l2[int(Num_d):]         # We need this step since we have used T.ones_like
...
weights_path = output_dir + 'weights_trained.mat'
# weights_trained.mat are the model weights that you will get after (or during) that training
